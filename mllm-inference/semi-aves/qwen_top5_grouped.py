...
This version uses a hierarchical prompt grouped by family. It does NOT assume
...
def get_name_family_from_row_or_id(row: pd.Series, k: int, id2meta: Dict[int, Dict[str, str]]) -> Tuple[Optional[str], Optional[str]]:
    """
    ...
    name = row[name_key] if name_key in row else None
    fam  = row[fam_key] if fam_key in row else None
    ...
    # Fallback via ID
    cid = row[id_key] if id_key in row else None
    if cid is not None:
        try:
            cid_int = int(cid)
            meta = id2meta.get(cid_int, {})
...
def build_hierarchical_prompt(row_like: Dict[str, Any], id2meta: Dict[int, Dict[str, str]]) -> str:
    """
    Build a family-grouped prompt.

    row_like: dict-like with possible keys:
      pred{k}, pred{k}_name, pred{k}_family for k in 1..5 (any subset may exist).
    If pred{k}_name/family are absent or empty, they are derived from pred{k] via id2meta.
    """
    # Make the access dict-like regardless of Series/dict input
    class RowShim(dict):
        def __getattr__(self, item): return self.get(item)
    row = RowShim(row_like)

    # Collect (name, family) for the 5 candidates with robust fallback
    name_fam_pairs: List[Tuple[Optional[str], Optional[str]]] = []
    for k in range(1, 6):
        nm, fam = get_name_family_from_row_or_id(row, k, id2meta)
        name_fam_pairs.append((nm, fam))

    # Group by family (Unknown bucket if family missing)
    grouped = defaultdict(list)
    for nm, fam in name_fam_pairs:
        if nm is None:
            continue
        grouped[fam or "Unknown"].append(nm)
    ...
    if not grouped:
        ...
        for i, (family, species_list) in enumerate(grouped.items(), start=1):
            plural = "species"  # intentionally same string for 1+; clearer and simpler
...
# ===================== Main =====================
def main():
    ap = argparse.ArgumentParser(description="Qwen Top-5 Explanations (TOPK_JSON) — Blueprint schema (hierarchical prompt).")
    ...
    # Materialize rows with pred1..5 (and conf1..5 if present; also pre-derive names/families when possible)
    records = []
    for rel in test_paths:
        ...
        for k in range(1, 6):
            ...
            # Pre-fill optional name/family (safe if absent in your upstream; purely internal to prompt)
            if cid is not None:
    ...
    if args.print_first_prompt:
        first_prompt = build_hierarchical_prompt(df.iloc[0].to_dict(), id2meta)
    ...
    for idx, (_, row) in enumerate(tqdm(df.iterrows(), total=len(df))):
        ...
        # --- Build the hierarchical prompt (robust to missing predX_name/family) ---
        prompt = build_hierarchical_prompt(row.to_dict(), id2meta)
