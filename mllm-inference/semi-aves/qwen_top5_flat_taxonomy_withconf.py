...
def _conf_str(p: Optional[float]) -> str:
    if p is None:
        return ""
    try:
        # assume 0..1; clamp just in case, and show two decimals
        p = max(0.0, min(1.0, float(p)))
        return f" [p={p:.2f}]"
    except Exception:
        return ""

def build_prompt(top5_ids: List[Optional[int]], top5_probs: List[Optional[float]], id2meta: Dict[int, Dict[str, str]]) -> str:
    """
    Build the explanation prompt with confidence shown as [p=..] next to each candidate.
    """
    entries: List[str] = []
    for i in range(5):
        cid = top5_ids[i] if i < len(top5_ids) else None
        prob = top5_probs[i] if i < len(top5_probs) else None
        if cid is None:
            name = "Unknown"
        else:
            meta = id2meta.get(int(cid), {})
            name = _format_species_full(meta)
        entries.append(f"{name}{_conf_str(prob)}")
    ...
    return (
        ...
        f"4. {entries[3]}\n"
        f"5. {entries[4]}\n\n"
         "Note on confidence: The confidence shown for candidate 1 (p1) reflects how certain the underlying model was. "
...
def main():
    ...
    # --- First prompt preview (now includes confidence) ---
    top5_ids_first = [df.iloc[0].get(f"pred{k}") for k in range(1, 6)]
    top5_probs_first = [df.iloc[0].get(f"conf{k}") for k in range(1, 6)]
    first_prompt = build_prompt(top5_ids_first, top5_probs_first, id2meta)
    ...
    for idx, (_, row) in enumerate(tqdm(df.iterrows(), total=len(df))):
        ...
        # --- Build the prompt with confidence included ---
        top5_ids = [row.get(f"pred{k}") for k in range(1, 6)]
        top5_probs = [row.get(f"conf{k}") for k in range(1, 6)]
        prompt = build_prompt(top5_ids, top5_probs, id2meta)
